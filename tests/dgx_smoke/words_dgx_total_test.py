#!/usr/bin/env python3
"""Submit 6 words to DGX using the NEW infer_dgx_total.sh (mimic + filter).

Same source videos as words_dgx_test.py / wordsl2_*. New jobs use prefix
`wtot_` to distinguish from prior runs. Output post-filter (pose-filtered)
mp4 instead of raw mimic.
"""
import asyncio
import os
import shutil
...
os.chdir(REPO_ROOT)
# New pipeline: mimic + filter in one sbatch
...
# Keep nodelist for now — total script has --exclude for 2 known-bad, but we
# also want autofs retry coverage on additional possibly-broken nodes.
...
async def main() -> int:
    staging = Path(tempfile.mkdtemp(prefix="words_total_"))
    ...
    out_dir = REPO_ROOT / "tests/dgx_smoke/out_words_dgx_total" / uuid.uuid4().hex[:8]
    out_dir.mkdir(parents=True, exist_ok=True)
    task_id = f"wtot_{uuid.uuid4().hex[:8]}"

    async def _progress(pct: float):
        print(f"      [{datetime.now().strftime('%H:%M:%S')}] progress={pct:.1f}%", flush=True)

    print(f"[2/3] submitting {len(TARGETS)} videos to DGX (mimic+filter pipeline)")
    print(f"      task_id  = {task_id}")
    print(f"      sbatch   = infer_dgx_total.sh  (FILTER_POSE=true default)")
    print(f"      output   = output_filter/input.mp4 (post-filter)")
    t0 = datetime.now()
    try:
    ...
    for r in report.get("results", []):
        ...
        if r.get("status") != "completed":
            ...
            continue
        db_id = f"wtot_{uuid.uuid4().hex[:8]}"
        ...
        row = Phase3TestJob(
            ...
            error_message=f"[words DGX TOTAL mimic+filter] sub={r.get('sub_task_id')} job={r.get('job_id')}",
    ...
    print(f"\n=== A/B URLS ===")
    print(f"DGX raw mimic   (wdgx6_*): https://auto.chatsign.ai/api/phase3-test/bundle/wdgx6")
    print(f"DGX mimic+filter (wtot_*): https://auto.chatsign.ai/api/phase3-test/bundle/wtot")
