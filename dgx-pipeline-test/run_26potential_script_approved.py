#!/usr/bin/env python3
"""26_potential_script approved (873) → DGX 4-stage chain (total → TG → SR).

Same flow as run_26comm02_approved.py for the 873 word-level recordings
that passed Heba's review (2026-05-07).
...
Per-sid pipeline (no cached mimic, all fresh):
    ...
    local: scp sr.mp4 → h264 remux → STAGING/<md5(c26ps_a_<sid>)[:10]>_hiya.mp4
...
Excludes:
    ...
    ADUAED21018WKLX24  (autofs broken 2026-05-08 — BASE_PY missing after 5 retries)
...
STATE = REPO / "dgx-pipeline-test" / "26_potential_script_approved_state.json"
STAGING = Path("/mnt/data/chatsign-phase3-test/26_potential_script-render-20260507/videos")
...
def fetch_one(rec):
    sid = rec["new_sid"]
    tid = task_id_for(rec)
    final = STAGING / video_filename(f"c26ps_a_{sid:04d}")
