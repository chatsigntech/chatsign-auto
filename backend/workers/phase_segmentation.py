...
def _build_annotations(
    ...
    # Deterministic shuffle so val set is representative
    ...
    logger.info(f"[{task_id}] Step 5.2: Annotations: {n_train} train, {n_val} val")
...
async def _train_model(
    ...
    logger.info(f"[{task_id}] Step 5.4: Training complete → {final_ckpt.name}")
    return final_ckpt
...
async def _run_segmentation(
    ...
    logger.info(f"[{task_id}] Step 5.5: Segmentation results → {result_file}")
    return result_file
...
async def run_phase_segmentation(
    ...
    if not manifest:
        raise RuntimeError("No sentence videos found in Phase 3 output")
    ...
    logger.info(f"[{task_id}] Phase 5: {video_count} sentence videos selected for training")
