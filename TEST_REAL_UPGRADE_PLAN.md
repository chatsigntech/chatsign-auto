...
设计要点：
- **断点续算自带**：`extract_clip_from_mp4.py:20 get_pending_videos()` 已经实现 skip-existing —— 扫输出目录已有 `<stem>_s2wrapping.npy`，只跑没跑过的；wrapper 不需自己实现
- **失败兜底**：损坏 / 0 帧的 mp4，由 `extract_one()` 内部打 warning 跳过；wrapper 可选地加一个 `failed.json` 汇总
- **进度日志**：`extract_clip_from_mp4.py` 已有 tqdm 进度条
...
```python
from spamo_segement.scripts.extract_features.extract_clip_from_mp4 import extract_features_from_mp4
ASL27K_VIDEOS = ...
ASL27K_FEATS = ...

extract_features_from_mp4(
    video_dir=ASL27K_VIDEOS,
    output_dir=ASL27K_FEATS,
    s2_mode="s2wrapping",
    scales=[1, 2],
...
| 数据源 | gloss 表现形态 | 例子 |
|---|---|---|
| orchestrator P1 输出（`chatsign-pipeline/pipeline.py:826`） | UPPER_UNDERSCORE | `MORE_THAN`, `ABU_DHABI` |
| **anno entry 的 `text` 字段**（`phase4_segmentation_train.py:321` `g.lower()` 拼接后） | **lowercase_underscore** | `"more_than home abu_dhabi"` |
| build_concat_aug 内部 `entry["text"].split()` | **lowercase_underscore** | `["more_than", "home", "abu_dhabi"]` |
...
**含义**：anno text 的 token 已经是 lowercase（phase4 worker 的 `.lower()` 转换），但仍带下划线。直接用 `_find_gloss_videos(token)` 对多词 token 全部 miss。需要 `_` → ` ` 转换：
...
def _normalize_for_asl27k(token: str) -> str:
    """Convert anno text token (lowercase_underscore) to ASL-27K csv key (lower with spaces).

    The token comes from build_concat_aug's `entry["text"].split()`, which reads
    the anno entry's text field. phase4_segmentation_train.py:321 already
    lowercased it via `.lower()`, so input here is e.g. 'more_than' not 'MORE_THAN'.
    ...
    NOTE: .strip().lower() is idempotent on already-lowercased input; we keep
    .lower() defensively in case of upstream format change. Do NOT call
    dataset_videos._find_gloss_videos directly — its lookup misses multi-word
    tokens because it doesn't replace underscores.
    """
...
def resolve_asl_resources(
    ...
    Returns:
        {
            "resources": {token: [(mp4_path, npy_path), ...]},   # key = lowercase_underscore (与 anno entry["text"].split() 对齐)
            "missing": [token, ...],                              # ASL-27K 没找到的词
            "feat_missing_files": [mp4_filename, ...],            # 视频找到但特征缓存缺失（debug 用）
            "n_glosses_hit": int,
            "n_clips_total": int,
        }

    IMPORTANT: caller 必须传入 lowercase_underscore 形式的 token list（从 base_anno
    的 entry["text"].split() 派生），否则 build_concat_aug 内部按原 token 查 dict
    会全部 miss。
    """
...
约 60 行。**dict key = lowercase_underscore**（与 build_concat_aug 内部 `entry["text"].split()` 直接对齐），不做目录化转换，无 `gloss.upper().replace()` 类潜在 bug。

**为什么不复用 `_find_gloss_videos`**：它内部 `gloss.strip().lower()` 不替换下划线，会让多词 token (如 `more_than`) 全部 miss。我们需要 `_normalize_for_asl27k` 做"下划线 → 空格"的关键转换，所以直接走 `_load_asl27k_gloss_map` 自己查。

**Step 3：`build_concat_aug` 接口**

移植后的 `backend/scripts/build_concat_aug.py` 在 P4 worker 内部从 base_anno 抽 token 后，**同时调** `resolve_asl_resources(tokens)` (C 类源) **和** `resolve_org_resources(tokens)` (B 类源) 拿到两个 dict，然后按 06b 算法生成 36x 拼接样本。具体改动详见 §3.3 Step 2。
...
- 拿一份典型 task 的 base_anno（跑过 `_generate_annotations()` 的 `anno_dir/{train,val,test}_info_ml.npy`），用 `_extract_tokens_from_anno()` 抽 unique tokens（lowercase_underscore），调 `resolve_asl_resources(tokens)`：
  - 检查 `n_glosses_hit / len(tokens) ≥ 0.8`（命中率门槛）
  - 检查 `feat_missing_files == []`（预热完整）
  - 抽 3 个命中词肉眼看 mp4 内容确实对应

> 不要直接拿 P1 `glosses.json`（UPPER_UNDERSCORE）做输入 —— 会让验证时使用的 token 大小写与运行时（lowercase from anno）不一致；实施层 bug 不会被验收捕获。
...
设计要点：
- 复用 SPAMO `extract_clip_from_mp4.py` 做实际提取（断点续算自带）
- **per-reviewer 循环**：accuracy uploads 分散在 5 个子目录（`Tareq/` `Heba/` `Rawdah/` `chatsign2026admin/` `test/`），而 `extract_clip_from_mp4.py` 是 `*.mp4` 单层 glob（line 25：`video_path.glob('*.mp4')`，**不递归**），需要 wrapper 在外层迭代每个 reviewer dir
...
for reviewer_dir in sorted(UPLOADS_DIR.iterdir()):
    if not reviewer_dir.is_dir():
        continue
    if args.reviewer and reviewer_dir.name != args.reviewer:
        continue
    out_dir = ORG_FEATS / reviewer_dir.name
    out_dir.mkdir(parents=True, exist_ok=True)
    logger.info(f"processing reviewer {reviewer_dir.name}")
    extract_features_from_mp4(
        video_dir=reviewer_dir,
        output_dir=out_dir,
        s2_mode="s2wrapping",
        scales=[1, 2],
        device=f"cuda:{args.gpu}",
...
def _normalize_for_org(token: str) -> str:
    """同 _normalize_for_asl27k：lowercase_underscore → lower with space.

    输入 token 来自 anno text（已 lowercased by phase4_segmentation_train.py:321）。
    例：'more_than' → 'more than'
    """
    return token.strip().lower().replace("_", " ")
...
- 用 §2.A.4 同样的 `_extract_tokens_from_anno()` 抽 lowercase_underscore tokens，调 `resolve_org_resources(tokens)`：
  - 检查 `n_glosses_hit / len(tokens) ≥ 0.4`（**门槛比 ASL-27K 低**：1806 vs 25440 词覆盖差距大）
  - 与 `resolve_asl_resources(tokens)` 比较 missing 列表，看 org 命中率是否符合预期
...
- **轻量**（保留 #3b helper 但禁用 B 类）：手工把 `_run_concat_aug` 的 `org["resources"]` 替换为 `{}` 空 dict 传给 `build_concat_aug` —— 触发自动降级到 21x preset（仅 C 类 asl_*）
- **彻底**（连 #3b helper 一起退）：`git revert` 包含 `from backend.scripts.org_resources import resolve_org_resources` 的 worker 改动；预热好的 `accuracy_word_uploads/*` 缓存保留不删（其他将来用途不影响）
...
**⚠️ 必须 1:1 保留的 test_real 06b 行为**（不保留就破坏算法 / 数据流）：

#### a) Deterministic RNG（不保留 → 不可复现）

实测 test_real `06b_build_concat_aug.py:206`：
```python
aug_rng = random.Random(args.aug_seed)   # 默认 args.aug_seed=1337
```
**单一全局 RNG**，C 类候选随机挑选 + 文本 derangement 都从这一个 RNG **顺序消费** —— 不是 per-sample md5(src+vp) 那种派生模式。

移植时保留：
- `--aug-seed` 参数（默认 1337）
- 单一全局 `random.Random(aug_seed)` 实例
- `--split-seed` 参数（默认 42，控制 train/val 划分，独立 RNG，line 299：`split_rng = random.Random(args.split_seed)`）
- **不要拆成 per-sample RNG**（消费顺序变化 → 输出变化）

可验证：连跑两次 `build_concat_aug` 同输入，`diff` 输出 npy + manifest 应当**字节级一致**。

#### b) Base sentences 来源 = test_info_ml.npy（C2）

test_real `06b:179-180`：
```python
test_base = np.load(args.base_anno / "test_info_ml.npy", allow_pickle=True)
sentences = list(test_base)
```

**只读 `test_info_ml.npy` 作为 base，不读 train/val**。原因：test_real 04 line 151 把 `test_info_ml` 当作"all sentences need inference later"（全部句子，不是 holdout）。

orchestrator 语义一致：`test_info_ml = current_entries`（current task 全部 sentences）。`train_pad`（用于凑训练量的 prev tasks / how2sign 数据）**不进入 aug 流程** —— 36x aug 后 current 自带数据量足够，pad 角色被替代。

移植代码 `for entry in sentences:` 严格 follow `test_info_ml.npy` 输入。

#### c) Leaky test (val_info_ml == test_info_ml)（C3）

test_real `06b:307`：
```python
test_arr = list(val_arr)   # test = val 副本，"leaky on purpose"
```

输出的 `aug_anno_dir/val_info_ml.npy` 与 `aug_anno_dir/test_info_ml.npy` 是**同一份数据**（leaky on purpose）。

实施提醒：
- 与 orchestrator 原 `anno_dir/test_info_ml`（current entries 用于 P5 推理）**同名但语义不同** —— aug 输出目录下的 test_info_ml 是 augmented val 副本
- P5 推理仍走原 `anno_dir/test_info_ml`，**不是** `aug_anno_dir/test_info_ml`
- aug 数据只用于 P4 SpaMo 训练；P5 推理用 base anno 的 test_info_ml

#### d) feat_out 末尾的 split symlinks（C1，**不建会让 P4 训练 crash**）

test_real `06b:351-356`：
```python
# Mirror the train/val/dev/test split symlinks created by 03_extract_clip_features.py
# so the dataloader can resolve <feat-dir>/<split>/*.npy. Without these the SpaMo
# how2sign Dataset bails with FileNotFoundError on Phase 4 train.
feat_out_abs = args.feat_out.resolve()
for split in ("train", "val", "dev", "test"):
    link = feat_out_abs / split
    if link.is_symlink() or link.exists():
        continue
    link.symlink_to(feat_out_abs)
```

orchestrator `phase4_segmentation_train.py:189-192` 已经为 base feat_dir 做了这步，但 **aug_feat_dir 是另一个目录，必须独立做这步**。否则 SpaMo dataloader `<feat_root>/<split>/<fid>.npy` lookup 失败。

#### e) 输出 npy 命名约定（C6）

test_real 命名（line 198, 235, 256, 274）：
```
<base_fid>_s2wrapping.npy        # base (symlink → base_feat 同名)
<base_fid>_a<n>_s2wrapping.npy   # A 类，第 n 个 shuffle (symlink → base_feat 同名)
<base_fid>_bv<v>_s2wrapping.npy  # B 类，第 v 个 variant (物理 npy)
<base_fid>_cv<v>_s2wrapping.npy  # C 类，第 v 个 variant (物理 npy)
```

移植**严格沿用**这一命名 —— SpaMo dataloader 按 entry 的 `fileid + "_s2wrapping.npy"` 找文件，命名不一致会找不到。

#### f) Audit 文件（C5，便于 debug）

test_real 写出（line 314, 346）：
- `aug_anno_dir/aug_recipe.json`：每个 fileid 详细记录 (cat / variant / order / used_uids / frame_count)
- `aug_anno_dir/build_summary.json`：总览 (counts / drop tallies / frame_stats by category)

移植保留这两份文件。aug 数据有问题时（命中率低、frame 数异常等），从这两份文件能快速定位。

#### g) 丢弃 dead code（D6）

test_real `06b:192-194` 计算 `base_feat_rel`：
```python
base_feat_rel = Path("..") / args.base_feat.relative_to(args.feat_out.parent) \
    if args.feat_out.parent in args.base_feat.parents \
    else args.base_feat
```

但实际后续 line 202-203 用绝对路径，`base_feat_rel` 没用上。**移植时直接用绝对路径**，不要带这段 dead code：
```python
...
def _extract_tokens_from_anno(base_anno: Path) -> list[str]:
    """从 test_info_ml.npy 抽出所有 unique token (lowercase_underscore).

    只读 test_info_ml（与 test_real 06b:179 一致）：
    - test_real 06b 把 test_info_ml 当作"全部待 augment 的 sentences"（不是机器学习
      意义上的 holdout test set，见 test_real 04 line 151 注释 "all sentences need
      inference later"）
    - orchestrator 的 test_info_ml = current_entries（current task 的所有 sentences），
      语义与 test_real 一致
    - build_concat_aug 内部也只 iterate test_info_ml；提取 token 范围必须与之一致

    格式：
    - P1 glosses.json 存 UPPER_UNDERSCORE
    - phase4_segmentation_train.py:321 用 g.lower() 转换后写入 anno text
    - build_concat_aug 的 entry["text"].split() 拿到的是 lowercase_underscore
    - resolve_*_resources 返回的 dict key 必须与 build_concat_aug 查询 token 严格一致
    """
    tokens = set()
    ...
    for entry in np.load(path, allow_pickle=True):
        ...
        tokens.update(text.split())   # already lowercase_underscore
...
async def _run_concat_aug(
    ...
    *, preset: str = "36x", val_fraction: float = 0.1, split_seed: int = 42,
    ...
    glosses = _extract_tokens_from_anno(base_anno)   # lowercase_underscore tokens
...
2. **升级后跑 colent + 默认 preset**：同一份 task 数据。worker 默认走 36x preset；如 ORG 命中率 < 40% 自动降级 21x（log 里会打 warning，记录最终 `effective_preset`）。记录 BLEU `BLEU_colent`
3. **门槛**：
   - 若 `effective_preset == "36x"`：`BLEU_colent ≥ BLEU_base + 5`（绝对值待实测，目标接近 94）
   - 若降级到 `21x`：`BLEU_colent ≥ BLEU_base + 4`（21x 比 36x 少 15 个 variant，期望略低）
...
按 help 输出对照 worker 现在的 `train_cmd` 参数（`--epochs 150 --batch-size 128 --hidden-dim 256 --block-size 6` 等）有无差异。通用 hyperparam 保留，**新增 / 重命名参数以实际 `--help` 输出为准**（本文档不预设具体名字）。
...
| ASL-27K / accuracy 数据路径不可访问 | `/mnt/data/chatsign-auto-videos` 或 `chatsign-accuracy/backend/data` 盘掉 / 权限问题 | `dataset_videos.py:_load_asl27k_gloss_map` 已有 fail-fast 警告，`asl_resources.py` 沿用；`org_resources.py:_build_org_index` 在 `UPLOADS_DIR.iterdir()` 找不到子目录时自然抛异常 |
...
- [ ] **#3a 跑通**：用 `_extract_tokens_from_anno(base_anno)` 从 task 的 anno 抽 lowercase_underscore tokens 列表，调 `resolve_asl_resources(tokens)` 满足 `n_glosses_hit / len(tokens) ≥ 0.8` 且 `feat_missing_files == []`
- [ ] **#3b 跑通**：同 token 列表调 `resolve_org_resources(tokens)` 满足 `n_glosses_hit / len(tokens) ≥ 0.4` 且 `feat_missing_files == []`
...
- `backend/workers/phase8_training.py` ← #2（两条脚本路径 + `proto_cmd` 加 `--skip-single` 参数；详见 §4.2）
